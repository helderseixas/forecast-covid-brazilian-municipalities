...
def epidemic_parameter_defuzzification(
        epidemic_periods_fuzzy_variable,
        epidemic_parameters_crisp_values,
        specif_day_in_time_series,
        fit_period_in_days=None,
        epidemic_parameter_in_forecast_period=None):
    if (epidemic_parameter_in_forecast_period is not None) and (specif_day_in_time_series >= (fit_period_in_days-1)):
        return epidemic_parameter_in_forecast_period
    else:
...
def sirds_model_fuzzy_kernel(t, y, *params):
    ...
    fit_period_in_days = params[4]
    fatality_in_forecast_period = params[5]
    contact_rate_in_forecast_period = params[6]
    loss_immunity_rate_in_forecast_period = params[7]

    # Fuzzy parameters
    beta = epidemic_parameter_defuzzification(
        beta_fuzzy_variable,
        beta_values,
        t,
        fit_period_in_days=fit_period_in_days,
        epidemic_parameter_in_forecast_period=contact_rate_in_forecast_period)

    f = epidemic_parameter_defuzzification(
        f_fuzzy_variable,
        f_values,
        t,
        fit_period_in_days=fit_period_in_days,
        epidemic_parameter_in_forecast_period=fatality_in_forecast_period)

    l = epidemic_parameter_defuzzification(
        l_fuzzy_variable,
        l_values,
        t,
        fit_period_in_days=fit_period_in_days,
        epidemic_parameter_in_forecast_period=loss_immunity_rate_in_forecast_period)
...
def sirds_model_fuzzy(parameters):
    ...
    fit_period_in_days = parameters[1]
    forecast_horizon_in_days = parameters[2]
    period_in_days = fit_period_in_days + forecast_horizon_in_days
    t = np.linspace(0, int(period_in_days - 1), int(period_in_days), dtype=int)

    fatality_in_forecast_period = parameters[3]
    contact_rate_in_forecast_period = parameters[4]
    loss_immunity_rate_in_forecast_period = parameters[5]

    #Initial SIRDS parameters
    days_between_infections = parameters[6]
    days_to_recovery = parameters[7]

    list_breakpoints_in_slow_transition = parameters[8]
    ...
    f_values = parameters[9:9+quantity_epidemic_periods_with_slow_transition]
    l_values = [1.0 / loss_immunity_in_days for loss_immunity_in_days in
                parameters[9+quantity_epidemic_periods_with_slow_transition:9+2*quantity_epidemic_periods_with_slow_transition]]

    num_breakpoints = int(len(parameters[9 + 2 * quantity_epidemic_periods_with_slow_transition:])/3)

    # Breakpoint parameters
    if num_breakpoints > 0:
        beta_values = [1.0 / days_between_infections for days_between_infections in parameters[9+2*quantity_epidemic_periods_with_slow_transition:9+2*quantity_epidemic_periods_with_slow_transition+num_breakpoints]]
        beta_breakpoint_values = parameters[9+2*quantity_epidemic_periods_with_slow_transition+num_breakpoints : 9+2*quantity_epidemic_periods_with_slow_transition+2*num_breakpoints]
        transition_days_between_epidemic_periods_values = parameters[9+2*quantity_epidemic_periods_with_slow_transition+2*num_breakpoints:]
    else:
        beta_values = []
...
def get_max_days_between_infections_in_outbreak_adjustment(date_begin,
                                                           date_begin_next_outbreak,
                                                           days_to_recovery,
                                                           df,
                                                           date_column,
                                                           effective_reproduction_number_column):
    df_period = df[(df[date_column] >= date_begin) & (df[date_column] < date_begin_next_outbreak)]
    if len(df_period) == 0:
